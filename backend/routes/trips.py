...
def list_trips():
    ...
    def add(cond, val):
        where.append(cond); params.append(val)

    if a.get("start_date"):     add("t.pickup_datetime >= %s", a["start_date"])
    if a.get("end_date"):       add("t.pickup_datetime <= %s", a["end_date"])
    if a.get("borough"):        add("pz.borough = %s", a["borough"])
    ...
    return jsonify({
        ...
        "sort_by": sort_by, "order": order.lower(), "data": data,
    })